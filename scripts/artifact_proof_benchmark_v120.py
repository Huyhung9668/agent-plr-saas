...
OUT = ROOT / "benchmarks" / "artifact_proof_compare_v120"
EXPORT_ROOT = ROOT / "exports" / "artifact_proof_compare_v120"
MAX_ROUNDS = 20
TARGET = 98
...
def fake_flags(text: str) -> list[str]:
    ...
    negators = ["no guaranteed", "not guaranteed", "not public launch ready", "without live proof", "without live payment", "do not claim", "do not promise", "not proven", "no income", "no sales result", "anyone wanting", "no ranking", "no fake", "avoid", "không claim", "khong claim", "không dùng", "khong dung", "không được", "khong duoc", "chưa được", "chua duoc"]
    for term in risky:
        pos = lower.find(term)
        if pos >= 0:
...
def score(answer: str, api_ok: bool, manifest: dict, *, is_agent: bool) -> tuple[dict[str, int], dict[str, str]]:
    ...
    scores["artifact_files"] = 10 if len(existing) >= 16 and total_artifact_words >= 2000 else 9 if len(existing) >= 15 and total_artifact_words >= 2000 else 8 if len(existing) >= 12 else 4
    ...
    scores["content_depth"] = 10 if depth_hits >= 10 and total_artifact_words >= 2200 else 9 if depth_hits >= 8 else 6
...
- Product files are materialized into `exports/artifact_proof_compare_v120/`.
...
## Files
- Benchmark folder: `benchmarks/artifact_proof_compare_v120/`
- Export folder: `exports/artifact_proof_compare_v120/`
- Latest round scorecard: `benchmarks/artifact_proof_compare_v120/round_{len(summary)}/scorecard.md`
"""
