...
OUT = ROOT / "benchmarks" / "artifact_proof_compare_v115"
EXPORT_ROOT = ROOT / "exports" / "artifact_proof_compare_v115"
MAX_ROUNDS = 20
TARGET = 98
...
def fake_flags(text: str) -> list[str]:
    ...
    negators = ["no guaranteed", "not guaranteed", "not public launch ready", "without live proof", "without live payment"]
    for term in risky:
        pos = lower.find(term)
        if pos >= 0:
            window = lower[max(0, pos - 100):pos + len(term) + 120]
...
def score(answer: str, api_ok: bool, manifest: dict, *, is_agent: bool) -> tuple[dict[str, int], dict[str, str]]:
    ...
    existing = [f for f in manifest.get("files", []) if f.get("words", 0) >= 40]
    ...
    scores["artifact_files"] = 10 if len(existing) >= 15 and total_artifact_words >= 2500 else 8 if len(existing) >= 12 else 4
    notes["artifact_files"] = f"files>=40 words={len(existing)}/16; artifact words={total_artifact_words}."
    ...
    scores["content_depth"] = 10 if depth_hits >= 10 and total_artifact_words >= 3500 else 9 if depth_hits >= 8 else 6
...
def main() -> None:
    ...
    for round_no in range(1, MAX_ROUNDS + 1):
        ...
        bj, be, blat = post_json("/api/chat", {"question": BASELINE_PROMPT + f"\nRound {round_no}: choose a fresh product angle.", "mode": "balanced", "tags": [], "agentKey": ""}, timeout=220)
...
- Product files are materialized into `exports/artifact_proof_compare_v115/`.
...
## Files
- Benchmark folder: `benchmarks/artifact_proof_compare_v115/`
- Export folder: `exports/artifact_proof_compare_v115/`
- Latest round scorecard: `benchmarks/artifact_proof_compare_v115/round_{len(summary)}/scorecard.md`
"""
