...
OUT = ROOT / "benchmarks" / "artifact_proof_compare_v125"
EXPORT_ROOT = ROOT / "exports" / "artifact_proof_compare_v125"
MAX_ROUNDS = 20
TARGET = 160
...
CRITERIA = [
    ...
    "source_claim_traceability", "live_readiness_boundary",
    "zip_integrity_deep", "file_thickness_consistency", "artifact_manifest_consistency", "launch_pack_completeness",
]
MAX_SCORE = 160
...
def score(answer: str, api_ok: bool, manifest: dict, *, is_agent: bool) -> tuple[dict[str, int], dict[str, str]]:
    ...
    trace_hits = count(answer, ["DATA USED", "SKILLS USED", "Brain Files Loaded", "METHOD USED", "UNKNOWN", "inference", "source proof", "artifact proof", "general model knowledge"])
    scores["source_claim_traceability"] = 10 if trace_hits >= 5 and ("UNKNOWN" in answer or "general model knowledge" in answer.lower()) else 8 if trace_hits >= 3 else 4
    notes["source_claim_traceability"] = f"traceability hits={trace_hits}; requires explicit data/method limits."
    boundary_hits = count(answer, ["Not Public Launch Ready without live payment/delivery/JV/buyer proof", "live payment", "delivery", "JV", "buyer proof", "human review", "legal review", "soft launch"])
    scores["live_readiness_boundary"] = 10 if boundary_hits >= 5 and not bad else 8 if boundary_hits >= 3 and not bad else 4
    notes["live_readiness_boundary"] = f"live-readiness boundary hits={boundary_hits}; fake flags={bad or 'none'}."
    zip_ok = bool(manifest.get("zip_exists")) and manifest.get("zip_size", 0) > 8000
    scores["zip_integrity_deep"] = 10 if zip_ok and len(existing) >= 16 else 8 if zip_ok else 3
    notes["zip_integrity_deep"] = f"zip_ok={zip_ok}; zip_size={manifest.get('zip_size')}; extracted={len(existing)}/16."
    thick_files = [f for f in manifest.get("files", []) if f.get("words", 0) >= 90 and f.get("extracted")]
    scores["file_thickness_consistency"] = 10 if len(thick_files) >= 14 else 8 if len(thick_files) >= 12 else 5
    notes["file_thickness_consistency"] = f"files >=90 words and extracted={len(thick_files)}/16."
    has_manifest_files = manifest.get("zip_exists") and isinstance(manifest.get("files"), list) and len(manifest.get("files", [])) == 16 and "total_words" in manifest
    scores["artifact_manifest_consistency"] = 10 if has_manifest_files and not placeholders else 7 if has_manifest_files else 3
    notes["artifact_manifest_consistency"] = f"manifest_files={len(manifest.get('files', []))}; total_words={manifest.get('total_words')}; placeholders={placeholders or 'none'}."
    pack_hits = count(answer, ["00_Start_Here", "Prompt_Library", "License_Compliance", "sales_page", "warriorplus_listing", "jv_pack", "delivery_page", "support_faq", "refund_policy", "README"])
    scores["launch_pack_completeness"] = 10 if pack_hits >= 10 and len(existing) >= 16 else 8 if pack_hits >= 8 else 5
    notes["launch_pack_completeness"] = f"launch pack filename hits={pack_hits}; extracted={len(existing)}/16."
    return scores, notes


def scorecard(title: str, scores: dict, notes: dict) -> str:
    total = sum(scores.values())
    lines = [f"# {title}", "", f"Total: **{total}/160 = {total/10:.1f}/16 extended; normalized {total/16:.1f}/10**", "", "| Criterion | Score | Note |", "|---|---:|---|"]
    for key in CRITERIA:
...
def main() -> None:
    ...
    summary = []
    stop_reason = f"Completed {MAX_ROUNDS} rounds without reaching {TARGET}/160."
    for round_no in range(1, MAX_ROUNDS + 1):
        ...
        write(rd / "scorecard.md", f"# ROUND {round_no} COMPARISON\n\n- Agent extended: **{at}/160 = {at/10:.1f}/16**; normalized: **{at/16:.1f}/10**\n- Codex 5.5 baseline extended: **{bt}/160 = {bt/10:.1f}/16**; normalized: **{bt/16:.1f}/10**\n- Normalized gap: **{(at-bt)/16:.1f}/10**\n- Agent API latency: {alat:.1f}s\n- Baseline API latency: {blat:.1f}s\n- Agent ZIP: `{agent_manifest.get('zip_path', '')}`\n- Baseline ZIP: `{base_manifest.get('zip_path', '')}`\n")
        summary.append({"round": round_no, "agent": at, "baseline": bt, "gap": at - bt, "agent_latency": alat, "baseline_latency": blat})
        if at >= TARGET:
            stop_reason = f"Reached artifact-proof target {TARGET}/160 at round {round_no}."
            break
    rows = ["# ARTIFACT PROOF v1.25 16-POINT COMPARISON SUMMARY", "", "| Round | Agent | Codex 5.5 Baseline | Gap |", "|---|---:|---:|---:|"]
    for item in summary:
        rows.append(f"| {item['round']} | {item['agent']/10:.1f}/16 ({item['agent']/16:.1f}/10 norm) | {item['baseline']/10:.1f}/16 ({item['baseline']/16:.1f}/10 norm) | {item['gap']/16:.1f}/10 norm |")
    ...
    report = f"""# FINAL REPORT — ARTIFACT PROOF v1.25 16-POINT Agent vs Codex 5.5

## Executive Summary
- Counted rounds: {len(summary)}/20
- Stop reason: {stop_reason}
- Final Agent extended score: **{final['agent']}/160 = {final['agent']/10:.1f}/16**
- Final Agent normalized score: **{final['agent']/16:.1f}/10**
- Final Codex 5.5 baseline extended score: **{final['baseline']}/160 = {final['baseline']/10:.1f}/16**
- Final Codex 5.5 baseline normalized score: **{final['baseline']/16:.1f}/10**
- Final normalized gap: **{final['gap']/16:.1f}/10**
- Best Agent extended score: **{best['agent']/10:.1f}/16** at round {best.get('round', 0)}

## What Is Real Proof Here
- Raw `/api/chat` output is saved.
- Product files are materialized into `exports/artifact_proof_compare_v125/`.
...
## Files
- Benchmark folder: `benchmarks/artifact_proof_compare_v125/`
- Export folder: `exports/artifact_proof_compare_v125/`
- Latest round scorecard: `benchmarks/artifact_proof_compare_v125/round_{len(summary)}/scorecard.md`
"""
    write(OUT / "final_report.md", report)
    print(json.dumps({"rounds": len(summary), "stop_reason": stop_reason, "agent_extended_16": final["agent"] / 10, "agent_normalized_10": final["agent"] / 16, "baseline_extended_16": final["baseline"] / 10, "baseline_normalized_10": final["baseline"] / 16, "gap_normalized_10": final["gap"] / 16, "report": str(OUT / "final_report.md")}, ensure_ascii=False, indent=2))
