...
OUT = ROOT / "benchmarks" / "artifact_proof_compare_v118"
EXPORT_ROOT = ROOT / "exports" / "artifact_proof_compare_v118"
MAX_ROUNDS = 20
TARGET = 98
...
- Each file section should have at least 60 words of usable content.
"""
...
def fake_flags(text: str) -> list[str]:
    ...
    negators = ["no guaranteed", "not guaranteed", "not public launch ready", "without live proof", "without live payment", "do not claim", "do not promise", "not proven", "no income", "no sales result", "anyone wanting", "no ranking", "no fake"]
    for term in risky:
        pos = lower.find(term)
        if pos >= 0:
            window = lower[max(0, pos - 180):pos + len(term) + 180]
...
- Product files are materialized into `exports/artifact_proof_compare_v118/`.
...
## Files
- Benchmark folder: `benchmarks/artifact_proof_compare_v118/`
- Export folder: `exports/artifact_proof_compare_v118/`
- Latest round scorecard: `benchmarks/artifact_proof_compare_v118/round_{len(summary)}/scorecard.md`
"""
