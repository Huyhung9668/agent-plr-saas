...
OUT = ROOT / "benchmarks" / "artifact_proof_compare_v124"
EXPORT_ROOT = ROOT / "exports" / "artifact_proof_compare_v124"
MAX_ROUNDS = 20
TARGET = 120
...
def scorecard(title: str, scores: dict, notes: dict) -> str:
    ...
    lines = [f"# {title}", "", f"Total: **{total}/120 = {total/10:.1f}/12 extended; normalized {total/12:.1f}/10**", "", "| Criterion | Score | Note |", "|---|---:|---|"]
    for key in CRITERIA:
...
def main() -> None:
    ...
    summary = []
    stop_reason = f"Completed {MAX_ROUNDS} rounds without reaching {TARGET}/120."
    for round_no in range(1, MAX_ROUNDS + 1):
        ...
        write(rd / "scorecard.md", f"# ROUND {round_no} COMPARISON\n\n- Agent extended: **{at}/120 = {at/10:.1f}/12**; normalized: **{at/12:.1f}/10**\n- Codex 5.5 baseline extended: **{bt}/120 = {bt/10:.1f}/12**; normalized: **{bt/12:.1f}/10**\n- Normalized gap: **{(at-bt)/12:.1f}/10**\n- Agent API latency: {alat:.1f}s\n- Baseline API latency: {blat:.1f}s\n- Agent ZIP: `{agent_manifest.get('zip_path', '')}`\n- Baseline ZIP: `{base_manifest.get('zip_path', '')}`\n")
        summary.append({"round": round_no, "agent": at, "baseline": bt, "gap": at - bt, "agent_latency": alat, "baseline_latency": blat})
        if at >= TARGET:
            stop_reason = f"Reached artifact-proof target {TARGET}/120 at round {round_no}."
            break
    rows = ["# ARTIFACT PROOF v1.24 12-POINT COMPARISON SUMMARY", "", "| Round | Agent | Codex 5.5 Baseline | Gap |", "|---|---:|---:|---:|"]
    for item in summary:
        rows.append(f"| {item['round']} | {item['agent']/10:.1f}/12 ({item['agent']/12:.1f}/10 norm) | {item['baseline']/10:.1f}/12 ({item['baseline']/12:.1f}/10 norm) | {item['gap']/12:.1f}/10 norm |")
    ...
    report = f"""# FINAL REPORT — ARTIFACT PROOF v1.24 12-POINT Agent vs Codex 5.5

## Executive Summary
- Counted rounds: {len(summary)}/20
- Stop reason: {stop_reason}
- Final Agent extended score: **{final['agent']}/120 = {final['agent']/10:.1f}/12**
- Final Agent normalized score: **{final['agent']/12:.1f}/10**
- Final Codex 5.5 baseline extended score: **{final['baseline']}/120 = {final['baseline']/10:.1f}/12**
- Final Codex 5.5 baseline normalized score: **{final['baseline']/12:.1f}/10**
- Final normalized gap: **{final['gap']/12:.1f}/10**
- Best Agent extended score: **{best['agent']/10:.1f}/12** at round {best.get('round', 0)}

## What Is Real Proof Here
- Raw `/api/chat` output is saved.
- Product files are materialized into `exports/artifact_proof_compare_v124/`.
...
## Files
- Benchmark folder: `benchmarks/artifact_proof_compare_v124/`
- Export folder: `exports/artifact_proof_compare_v124/`
- Latest round scorecard: `benchmarks/artifact_proof_compare_v124/round_{len(summary)}/scorecard.md`
"""
    write(OUT / "final_report.md", report)
    print(json.dumps({"rounds": len(summary), "stop_reason": stop_reason, "agent_extended_12": final["agent"] / 10, "agent_normalized_10": final["agent"] / 12, "baseline_extended_12": final["baseline"] / 10, "baseline_normalized_10": final["baseline"] / 12, "gap_normalized_10": final["gap"] / 12, "report": str(OUT / "final_report.md")}, ensure_ascii=False, indent=2))
