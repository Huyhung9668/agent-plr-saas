# Export ZIP + Test ZIP / Delivery Flow

## Purpose
Điều phối và sinh nội dung cho Step 36: Export ZIP + Test ZIP / Delivery Flow trong hệ thống WarriorPlus AI Printables/KDP/PLR Product Builder.

## When To Use
Dùng khi user gọi rõ `Step 36` / `Bước 36` hoặc dùng intent tự nhiên khớp với skill này.
...
1. Xác nhận explicit step = 36 hoặc keyword match.
...
5. Chạy AI/tool theo route type `tool_action`.
6. Ghi output/missing assets/quality gate vào response và debug.

## Output Schema
```txt
PRODUCT USED:
STEP USED: 36 — Export ZIP + Test ZIP / Delivery Flow
FILES USED:
...
NEXT FIX:
REQUEST DEBUG:
- selected_skill: 36_export_zip_test_delivery_flow
...
## Files To Read
- `project_state.json`
- Active project markdown/csv/txt files needed for Step 36

## Files To Create/Update
- `final_product.zip`

## Quality Checklist
- [ ] Đúng Step 36, không route sang step khác.
...
Step 36 Export ZIP + Test ZIP / Delivery Flow cho product hiện tại.`
- `export zip cho active project, chấm điểm và nêu missing assets.`
- `Hỏi Step 36 mà trả step khác là lỗi gì? Debug route, không dùng mẫu cũ.`
