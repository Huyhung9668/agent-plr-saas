# Placeholder Check + Export Manifest

## Purpose
Điều phối và sinh nội dung cho Step 35: Placeholder Check + Export Manifest trong hệ thống WarriorPlus AI Printables/KDP/PLR Product Builder.

## When To Use
Dùng khi user gọi rõ `Step 35` / `Bước 35` hoặc dùng intent tự nhiên khớp với skill này.
...
1. Xác nhận explicit step = 35 hoặc keyword match.
...
5. Chạy AI/tool theo route type `hybrid_action`.
6. Ghi output/missing assets/quality gate vào response và debug.

## Output Schema
```txt
PRODUCT USED:
STEP USED: 35 — Placeholder Check + Export Manifest
FILES USED:
...
NEXT FIX:
REQUEST DEBUG:
- selected_skill: 35_placeholder_check_export_manifest
- route_type: hybrid_action
- api_called:
- from_cache: false
...
## Files To Read
- `project_state.json`
- Active project markdown/csv/txt files needed for Step 35
...
## Quality Checklist
- [ ] Đúng Step 35, không route sang step khác.
...
Step 35 Placeholder Check + Export Manifest cho product hiện tại.`
- `placeholder check cho active project, chấm điểm và nêu missing assets.`
- `Hỏi Step 35 mà trả step khác là lỗi gì? Debug route, không dùng mẫu cũ.`
